# Module 29 : ROP Chains ARM64

## 🎯 Ce que tu vas apprendre

- Comprendre le ROP (Return-Oriented Programming) sur ARM64
- Construire des chaînes ROP pour bypasser W^X et DEP
- Exploiter les gadgets ARM64 spécifiques
...
Le ROP est une technique d'exploitation qui consiste à **enchaîner des petits morceaux de code existant** (appelés "gadgets") pour exécuter du code arbitraire **sans injecter de nouveau code**.

**Pourquoi ?**

Les protections modernes empêchent l'exécution de code sur la stack ou la heap (W^X / DEP). Le ROP contourne cela en réutilisant du code déjà présent dans des sections exécutables.

**Comment ?**

On manipule la stack pour enchaîner des adresses de gadgets. Chaque gadget se termine par `RET` (ou `BR X30` sur ARM64), ce qui charge l'adresse suivante depuis la stack.
...
STACK APRÈS OVERFLOW (ROP Chain) :
┌──────────────┐
│ "AAAA..." x64│ ← Remplir buffer
├──────────────┤
│ Gadget 1     │ ← pop x0, ret
├──────────────┤
│ Argument 1   │ ← Valeur pour X0
├──────────────┤
│ Gadget 2     │ ← pop x1, ret
├──────────────┤
│ Argument 2   │ ← Valeur pour X1
├──────────────┤
│ Gadget 3     │ ← syscall
└──────────────┘
```
...
Un gadget est une **séquence d'instructions terminée par un retour** (`RET`, `BR X30`, `LDR PC, [SP]`, etc.).

**Pourquoi c'est différent sur ARM64 ?**

ARM64 utilise une convention d'appel différente :
- Registres arguments : X0-X7
- Link Register : X30 (LR) contient l'adresse de retour
- `RET` = `BR X30` (saute à l'adresse dans LR)
...
PAC est une protection matérielle d'Apple Silicon qui **signe cryptographiquement** les pointeurs (notamment LR) pour détecter leur corruption.

**Pourquoi c'est un problème pour ROP ?**

Quand on écrase LR sur la stack, la signature PAC devient invalide. Au moment du `RET`, le CPU vérifie la signature et plante si elle est incorrecte.

**Comment contourner ?**

1. **Gadgets non-PAC** : Certaines fonctions ne signent pas LR
2. **Leak PAC** : Si on peut lire la stack, on récupère la signature
3. **Brute force** : PAC utilise seulement 16 bits (~65k tentatives)
4. **Gadgets BR X** : Sauter vers un registre contrôlable au lieu de LR
...
│ gadget_1    │
│   pop x0    │
│   ret       │ ← Charge adresse suivante depuis stack
└─────────────┘
       ↓
┌─────────────┐
│ gadget_2    │
...
1. Préparer arguments :
   X0 = adresse page
   X1 = taille
   X2 = PROT_READ | PROT_WRITE | PROT_EXEC

2. Charger numéro syscall :
...
Construire une chaîne ROP pour exécuter `/bin/sh`.

```python
#!/usr/bin/env python3
import struct

# Adresses des gadgets (à trouver avec ROPgadget ou radare2)
...
// Adresses des gadgets (à adapter)
...
### Exemple 4 : Bypass PAC avec Gadgets Non-Signés

Chercher des fonctions qui n'utilisent pas PAC.

```bash
# Désassembler et chercher fonctions sans PACIBSP
objdump -d programme | grep -A5 "^[0-9a-f]* <"

# Fonctions avec PAC :
100001000 <main>:
100001000: pacibsp        ; ← Signe LR
100001004: stp x29, x30, [sp, #-16]!
...
1000010fc: ldp x29, x30, [sp], #16
100001100: retab          ; ← Vérifie signature

# Fonctions SANS PAC (cibles pour ROP) :
100002000 <helper>:
100002000: stp x29, x30, [sp, #-16]!  ; ← Pas de PAC !
...
100002010: ldp x29, x30, [sp], #16
100002014: ret             ; ← RET normal, pas RETAB
```

## 🎯 Application Red Team

### Scénario 1 : Exploit macOS avec ROP
...
# (bibliothèque toujours chargée)
LIBSYSTEM_BASE = 0x7ff800000000  # À leak via ASLR
...
### Scénario 2 : ROP Automatisé avec pwntools
...
# Construire automatiquement la chaîne
...
def bruteforce_pac(base_addr):
    ...
    for pac_bits in range(0x10000):
        ...
        # Tester (le programme crash ou réussit)
        ...
        try:
            ...
            if result:
                print(f"PAC trouvé : {pac_bits:04x}")
                return addr
        except:
            continue
...
1. **ROP = réutiliser du code existant** pour contourner W^X/DEP
2. **Gadgets ARM64** : chercher `ldp x29, x30, [sp], #16 ; ret`
3. **PAC sur Apple Silicon** : protège LR, mais peut être contourné
4. **Outils** : ROPgadget, radare2, pwntools pour automatiser
5. **Cibles** : libc, libSystem.dylib (toujours chargées en mémoire)
6. **Techniques** : leak ASLR, bruteforce PAC, gadgets non-signés
